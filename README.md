...
总程序会编排连接设备、打开 MFC2 载气、等待稳定、启动数据记录、通氢、恢复和清理，并输出每轮 CSV 路径。分析和绘图由 agent 另行调用 `analysis/` 下的脚本完成。建议先 dry-run 查看计划：
...
```bash
python cli_tools/experiment_cli.py run "进行十次4%氢气测试，每次40秒，使用功率计测量" --mfc-port COM3 --sensor-name sensor_A
```
