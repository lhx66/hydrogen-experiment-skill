...
总程序会编排连接设备、打开 MFC2 载气、等待稳定、启动数据记录、默认预记录 2 秒后打开 MFC1 氢气、参数化通氢流程、恢复和清理，并输出每轮 CSV 路径。2 秒预记录包含在每轮记录总时长内，不会额外延长用户要求的记录时长。分析和绘图由 agent 另行调用 `analysis/` 下的脚本完成。CLI 不接受自然语言位置参数；agent 需要先把实验需求转换为 `--loop-count`、`--instrument` 和一个或多个 `--step`。建议先 dry-run 查看计划：
...
```bash
python cli_tools/experiment_cli.py run --output-folder "E:\experiments\2026-06-17_sensor_A" --mfc-port COM3 --sensor-name sensor_A --instrument powermeter --loop-count 10 --step h2:4:40
```

如果用户未要求更换数据文件夹，后续命令可以省略 `--output-folder`，总程序会沿用上次实验数据文件夹：

```bash
python cli_tools/experiment_cli.py run --mfc-port COM3 --sensor-name sensor_A --instrument powermeter --loop-count 10 --step h2:4:40
```

复杂流程示例：每轮先 3% 氢气 20 秒，关闭 MFC1 等待 10 秒，再 2% 氢气 30 秒，循环 5 次：
...
# 数据分析，--json 输出便于 agent 解析
python analysis/analyze_sensor_response.py analyze data.csv --json

# 响应曲线绘图，建议保存PNG并报告路径
python analysis/plot_sensor_response.py data.csv --output data_response.png --title "Cycle 1"
```
...
1. Agent 解析用户的实验需求，并转换为参数化 CLI 命令；不要把自然语言句子传给 `experiment_cli.py`
...
6. 执行每个循环：
   - 先启动功率计或 FBG 数据采集
   - 开始记录后默认等待 2 秒再打开 MFC1 氢气；这 2 秒包含在本轮记录总时长内
   - 按 `--step` 顺序执行通氢和等待步骤
   - `h2` 步骤设置 MFC1 流量并保持指定时间，随后关闭 MFC1
   - `wait` 步骤保持 MFC1 关闭并等待指定时间
   - 继续采集恢复段直到本轮总记录时长结束
   - 输出本轮 CSV 路径，供 agent 调用分析脚本
7. 非最后一轮时等待循环间隔，默认 60 秒
8. 关闭所有设备：先关 MFC1，再关 MFC2
9. agent 对每轮 CSV 调用 `analyze_sensor_response.py analyze <csv> --json`，按固定格式报告响应指标
10. 所有循环结束后，agent 调用 `plot_sensor_response.py <csv...> --output <allcycles.png>` 保存汇总图并报告路径
11. 打印实验 JSON 到 agent 窗口；用户明确要求保存实验结果时，使用 `save_artifacts=True` 保存 JSON 并报告路径

自动流程生成的文件名不添加时间戳，改为包含关键实验信息，例如 `sensor_A_H2-3percent_MFC1-30sccm_MFC2-1slm_H2time-40s_Record-70s_FBG-ch1_cycle01.csv`。最终汇总响应曲线图由 agent 调用绘图脚本生成，建议使用同一组关键信息并追加 `allcycles` 标识；用户要求保存实验结果时，JSON 追加 `results` 标识。
