...
总程序会编排连接设备、打开 MFC2 载气、等待稳定、启动数据记录、参数化通氢流程、恢复和清理，并输出每轮 CSV 路径。分析和绘图由 agent 另行调用 `analysis/` 下的脚本完成。CLI 不接受自然语言位置参数；agent 需要先把实验需求转换为 `--loop-count`、`--instrument` 和一个或多个 `--step`。建议先 dry-run 查看计划：
