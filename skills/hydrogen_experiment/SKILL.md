...
AI必须先引导用户补齐并确认实验信息，不能直接连接设备、打开 MFC 或启动采集。**必须先确认实验结果保存文件夹、传感器名称、MFC串口、测量仪器、循环次数和参数化通氢流程**。

必须确认或采用默认值：
- 实验结果保存文件夹：首次必须询问；之后用户未指定新文件夹时，**默认沿用上次实验数据文件夹**。
- 传感器名称、MFC串口、测量仪器、循环次数、参数化通氢流程。
- 参数化通氢流程：`h2:<浓度%>:<秒>` 表示通氢步骤，`wait:<秒>` 表示关闭 MFC1 后等待。例：`--step h2:3:20`；复杂流程例：`--loop-count 5 --step h2:3:20 --step wait:10 --step h2:2:30`。
...
文件名不添加时间戳；文件夹名称通常由用户指定并可包含日期。文件名必须包含关键实验信息。单轮 CSV 示例：
...
| 脚本 | 阶段 | 典型命令 |
| --- | --- | --- |
| `cli_tools/experiment_cli.py` | 阶段2-4 | `python cli_tools/experiment_cli.py run --output-folder "E:\experiments\2026-06-17_sensor_A" --mfc-port COM3 --sensor-name sensor_A --instrument fbg --loop-count 3 --step h2:3:20 --dry-run` |
