...
| 参数 | 必填/可选 | 默认值 | 说明 |
|------|-----------|--------|------|
| 实验结果保存文件夹 | 必填（首次） | 当前工作区内 | 首次必须询问，后续默认沿用上次实验数据文件夹 |
| 传感器名称 | 必填 | - | - |
| MFC串口 | 可选 | - | 默认运行 `python cli_tools/mfc_cli.py connect --list` 检查串口端口号 |
| 测量仪器 | 必填 | - | `fbg` 或 `powermeter` |
| 循环次数 | 必填 | - | - |
| 氢气浓度 | 必填 | - | 超过 4% 需要明确授权 |
| 通氢时间 | 必填 | - | 每段通氢时长 |
| 记录时长 | 可选 | 300 秒 | - |
| 组间间隔 | 可选 | 5 秒 | - |
| MFC2载气流量 | 可选 | 1.0 slm | 注:MFC1 流量(sccm) = 氢气浓度(%) × MFC2 流量(slm) × 10 |
| FBG 通道 | 可选 | 通道 1 | 仅在使用 FBG 时适用 |
