...
- 常规一组实验默认是：打开记录、通入氢气一定时间、关闭 MFC1、结束记录或进入恢复段。常规实验优先只询问氢气浓度和通氢时间，并由 agent 转换为单个 `--step h2:<浓度%>:<秒>`。
- 只有用户要求多段通氢或等待时，才展开询问参数化通氢流程。参数化通氢流程使用 `h2:<浓度%>:<秒>` 和 `wait:<秒>`；复杂流程例：`--loop-count 5 --step h2:3:20 --step wait:10 --step h2:2:30`。
