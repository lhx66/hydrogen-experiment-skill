...
class HydrogenExperimentAsyncFlowTests(unittest.TestCase):
    ...
    def test_cycle_postprocessing_is_external_to_async_runner(self):
        signature = inspect.signature(self.module.HydrogenExperimentSkill.start_experiment)
        self.assertNotIn("analyze_cycles", signature.parameters)
        self.assertNotIn("plot_cycles", signature.parameters)

    def test_finalize_lists_cycle_files_without_generating_plot(self):
        output_dir = ROOT / "tmp_test_output" / "async_finalize_csv_only"
    ...
    def test_final_json_is_displayed_by_default(self):
